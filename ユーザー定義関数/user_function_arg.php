...
//6.3.6 可変長引数で「1個以上に引数を渡す方法」
/*
可変長引数は、より正しくは「0個以上の引数」を表します。
*/
//最低でも一つの引数を必要とする場合
function total_6(float ...$args): float {
    if( count($args) === 0 ){
        throw new Exception('引数は一個以上指定してください');
    }
    $result = 0;
    foreach($args as $arg){
        $result += $arg;
...
/*
しかし、このような関数は最善とは言えません。
この関数が1つ以上の引数を要求していることは、中身のコードを読み解かなければわからないからです。
そこで引数を1つ受け取ることは確実なので、1個目の引数は（可変長でない）普通の引数$initとして宣言し、
2個目以降の引数を可変長引数として宣言するわけです。
*/
function total_6_1(float $init, float ...$args): float {
    $result = $init;
    foreach($args as $arg){
        $result += $arg;
    }
